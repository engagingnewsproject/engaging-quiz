...
class Enp_quiz_Dashboard extends Enp_quiz_Create {
    ...
    // public function get_quiz_dashboard_item_title_test($quiz)
    // {
    //  if (!is_object($quiz)) {
    //      return false;
    //  }
    //  $quiz_status = $quiz->get_quiz_status();
    //  if ($quiz_status === 'published') {
    //      $quiz_title_test = '<a href="' . ENP_QUIZ_RESULTS_URL . $quiz->get_quiz_id() . '">' . $quiz->get_quiz_title_test() . '</a>';
    //  } elseif ($quiz_status === 'draft') {
    //      $quiz_title_test = '<a href="' . ENP_QUIZ_CREATE_URL . $quiz->get_quiz_id() . '"><span class="enp-screen-reader-text">Edit </span>' . $quiz->get_quiz_title_test() . '</a>';
    //  }
